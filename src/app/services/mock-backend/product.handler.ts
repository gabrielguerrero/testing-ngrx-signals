...
export const productHandlers = [
  http.post<never, { productId: string; quantity: number }[], string>(
    '/checkout',
    () => {
      return HttpResponse.json('123');
    },
  ),
  http.get<{ id: string }, never, ProductDetail | undefined>(
    '/products/:id',
    ({ params }) => {
      const id = params.id;
  ...
  http.get<Required<ProductsQuery>, never, ProductsResponse>(
    '/products',
    ({ request }) => {
