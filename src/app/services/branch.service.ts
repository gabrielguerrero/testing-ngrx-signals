...
import { Branch, BranchDetail, Product } from '../models';
import { getRandomInteger, sortData } from './mock-backend/utils';
...
export class BranchService {
  ...
  getBranches(options?: {
    ...
    console.log('BranchService.getBranches', options);
    if (options?.search)
    ...
    return of({
      resultList: result,
      total,
      hasMore: (options?.skip || 0) + result.length < total,
    }).pipe(delay(500));
  }
}
